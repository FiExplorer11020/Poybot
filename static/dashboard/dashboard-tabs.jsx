...
// ─── WALLET GRAPH (V3 "univers") ──────────────────────────────────────────────
// Replaces the V1 SVG (slow, dated) and V2 Cosmograph (rigid styling). The
// new wallet graph is implemented in 3 IIFE files under static/dashboard/
// walletgraph_v3/ (BackgroundAmbient, Graph, SelectionPanel) and exported on
// window.WG_V3.*. This wrapper composes them, keeping the existing KPI strip
// and filters above the canvas.
const WalletGraph = () => {
  const { snapshot } = useLiveStore();
  const [selectedWallet, setSelectedWalletState] = useStateT(null);
  if (typeof window !== 'undefined' && window.__LIVESTORE_DEBUG__) {
    console.log('[re-render] WalletGraph (V3)');
  }
  const stats = snapshot?.wallet_graph?.stats || {};

  const WGv3 = (window.WG_V3 || {}).WalletGraphV3;
  const Panel = (window.WG_V3 || {}).SelectionPanel;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* KPI strip (unchanged) */}
      <KpiStrip items={[
        { label: 'Leaders', value: fmtNum(stats.leaders) },
        { label: 'Followers', value: fmtNum(stats.followers) },
        { label: 'Edges total', value: fmtNum(stats.edges_total) },
        { label: 'Edges confirmed', value: fmtNum(stats.edges_confirmed) },
        { label: 'Active 24h', value: fmtNum(stats.active_24h) },
        { label: 'Phase 2+', value: fmtNum(stats.phase_2_plus) },
      ]} />

      {/* Graph canvas + selection panel overlay */}
      <div style={{ flex: 1, position: 'relative', minHeight: 0, background: '#06080f' }}>
        {WGv3 ? (
          <WGv3 onSelect={setSelectedWalletState} />
        ) : (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#94a8d6', fontFamily: 'JetBrains Mono, monospace', fontSize: 11 }}>
            Loading WalletGraph V3…
          </div>
        )}
        {Panel && selectedWallet && (
          <Panel wallet={selectedWallet} onClose={() => setSelectedWalletState(null)} />
        )}
      </div>
    </div>
  );
