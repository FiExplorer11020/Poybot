...
class PolymarketWsIngestor:
    def __init__(self, ws_url: str, session_factory: async_sessionmaker, token_ids: list[str]) -> None:
        self.ws_url = ws_url
        ...
        async with self.session_factory() as session:
            ...
            for payload in payloads:
                ...
                if payload.get("event_type") == "book":
                    bid = Decimal(str(payload.get("best_bid", 0))) if payload.get("best_bid") is not None else None
                    ask = Decimal(str(payload.get("best_ask", 0))) if payload.get("best_ask") is not None else None
                    await repo.insert_top_of_book(
