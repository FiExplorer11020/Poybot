...
- Memory caps that fit a 2 GB VM with headroom: postgres 300 MB,
  redis 64 MB, observer 300 MB, engine 600 MB, registry 200 MB,
  api 200 MB.
...
- **Image registry push.** No `docker push` step yet — added in S5.13
  when we provision the Oracle VM.
- **Secrets.** `.env` is bind-mounted via `env_file:`. Move to docker
  secrets / sops when we go live.
- **Backups.** Postgres backups → Cloudflare R2 land in S4.12.
